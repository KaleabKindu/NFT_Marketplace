...
namespace Application.Features.Assets.Dtos.Validators
{
    public class CreateAssetDtoValidator : AbstractValidator<CreateAssetDto>
    {
        public CreateAssetDtoValidator()
        {
            ...
            RuleFor(dto => dto.CreatorId).GreaterThan(0).WithMessage("Valid CreatorId is required.");
            ...
            RuleFor(dto => dto.IpfsHash).NotEmpty().WithMessage("IPFS hash is required.");
            RuleFor(dto => dto.MintedAt).NotEmpty().WithMessage("MintedAt is required.");
