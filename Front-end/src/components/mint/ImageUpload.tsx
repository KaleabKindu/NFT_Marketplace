      ...
      <FormItem>
        ...
        <FormControl >
          <div 
            ...
            onDragCapture={(e) => {e.preventDefault(); console.log('capture')}} 
            onDragLeave={(e) => {e.preventDefault(); console.log('leave')}} 
          ...
          >
              ...
              <>
                ...
                <Input ref={ref} type='file' className='hidden' onChange={(e) => field.onChange(e.target.files?.[0])}/>
              </>:
              <>
                {field.value.type.startsWith('image') && 
                <div className='relative mx-auto w-full h-full'>
                  <Image className='rounded-2xl  object-cover' src={URL.createObjectURL(field.value)} fill alt='' />
                </div>}
                {field.value.type.startsWith('video') && 
                  <div className='relative mx-auto lg:w-[60%] h-full'>
                    <video controls className='absolute h-full w-full' src={URL.createObjectURL(field.value)} />
                  </div>
                }
                {/* <TypographyP text={field.value.name}/> */}
                <Button type='button' variant='ghost' size='icon' className='absolute right-5 top-5 rounded-full' onClick={() => form.setValue('image', null)}>
                  <MdDelete size={25}/>
                </Button>
              </>
              }
